...
export function useNotificationActions() {
  ...
  // Example React Query mutations
  const acceptFriendRequest = useMutation({
    mutationFn: (id) => axiosInstance.post(`/friends/accept/${id}`),
    onSuccess: () => {
  ...
  const declineFriendRequest = useMutation({
    mutationFn: (id) => axiosInstance.post(`/friends/decline/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries(["notifications"]);
    },
  });
              ...
              {results.map((notif) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Render actions dynamically if available */}
                    {actions.accept && (
                      <div className="flex gap-3 mt-2">
                        <Button
                          onClick={() => actions.accept(notif.id)}
                 ...
                 hover:shadow-[0_0_8px_rgba(168,85,247,0.4)]"
                        ...
                        <Button
                          onClick={() => actions.decline(notif.id)}
