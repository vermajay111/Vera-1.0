import * as React from "react";
import { useSelector } from "react-redux";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
  SidebarGroup,
  SidebarGroupContent,
  SidebarGroupLabel,
  SidebarHeader,
  SidebarMenu,
  SidebarMenuButton,
  SidebarMenuItem,
  SidebarRail,
} from "@/components/ui/sidebar";
import {
  Command,
  LogIn,
  UserPlus,
  Accessibility,
  Globe,
  Send,
  CircleHelp,
  Database,
  ShieldAlert,
  Users,
  BellRing,
  Newspaper,
  UserRoundPlusIcon,
} from "lucide-react";
import { Friends } from "../friends/friends";
import { Notifcations } from "../notifications/notifications";
import { NavUser } from "./nav-user";


const auth_buttons = {
  navMain: [
    {
      title: "Getting Started",
      items: [
        { title: "Start New Promise", url: "/start", icon: Newspaper },
        { title: "Browse Friends", Wrapper: Friends, icon: UserRoundPlusIcon },
      ],
    },
    {
      title: "Management",
      items: [
        { title: "My Promises", url: "/start", icon: ShieldAlert },
        { title: "My Friends", url: "/friends", icon: Users },
        { title: "Notifications", Wrapper: Notifcations, icon: BellRing },
      ],
    },
    {
      title: "Help",
      items: [
        { title: "Support", url: "#", icon: Globe },
        { title: "Feedback", url: "#", icon: Send },
        { title: "FAQ", url: "#", icon: CircleHelp },
        { title: "Accessibility", url: "#", icon: Accessibility },
      ],
    },
  ],
  navFooter: [
    {
      title: "Account Management",
      items: [{ title: "Settings" }],
    },
  ],
};

const guest = {
  navMain: [
    {
      title: "Getting Started",
      items: [
        { title: "Login", url: "/login", icon: LogIn },
        { title: "Signup", url: "/signup", icon: UserPlus },
      ],
    },
    {
      title: "Help",
      items: [
        { title: "Support", url: "#", icon: Globe },
        { title: "Feedback", url: "#", icon: Send },
        { title: "FAQ", url: "#", icon: CircleHelp },
        { title: "Accessibility", url: "#", icon: Accessibility },
      ],
    },
    {
      title: "Your Data",
      items: [{ title: "Data Info", url: "#", icon: Database }],
    },
  ],
  navFooter: [
    {
      title: "Account Management",
      items: [{ title: "Settings" }],
    },
  ],
};

export function AppSidebar(props) {
  const username = useSelector((state) => state.user.value);
  const token = useSelector((state) => state.token.refresh)
  console.log(token)

  const user_info = {
    name: username,
    email: username ? `Welcome back, ${username}` : "",
    avatar: "/avatars/shadcn.jpg",
  };

  const data = (username && token) ? auth_buttons : guest;
  return (
    <Sidebar {...props}>
      {/* -------- Header -------- */}
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton size="lg" asChild>
              <a
                href="/home"
                className="flex items-center gap-2 cursor-pointer"
              >
                <div className="bg-sidebar-primary text-sidebar-primary-foreground flex aspect-square size-8 items-center justify-center rounded-lg">
                  <Command className="w-5 h-5" />
                </div>
                <div className="grid flex-1 text-left text-base leading-tight">
                  <span className="truncate font-medium">Vera</span>
                  <span className="truncate text-sm text-gray-400">
                    Keep Your Word
                  </span>
                </div>
              </a>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
      </SidebarHeader>

      {/* -------- Main Content -------- */}
      <SidebarContent>
        {data.navMain.map((section) => (
          <SidebarGroup key={section.title}>
            <SidebarGroupLabel className="text-base">
              {section.title}
            </SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
                {section.items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    {item.Wrapper ? (
                      <item.Wrapper>
                        <SidebarMenuButton asChild>
                          <div className="flex items-center gap-2 text-base cursor-pointer">
                            <item.icon className="w-4 h-4" />
                            <span>{item.title}</span>
                          </div>
                        </SidebarMenuButton>
                      </item.Wrapper>
                    ) : (
                      <SidebarMenuButton asChild>
                        <a
                          href={item.url}
                          className="flex items-center gap-2 text-base cursor-pointer"
                        >
                          <item.icon className="w-4 h-4" />
                          <span>{item.title}</span>
                        </a>
                      </SidebarMenuButton>
                    )}
                  </SidebarMenuItem>
                ))}
              </SidebarMenu>
            </SidebarGroupContent>
          </SidebarGroup>
        ))}
      </SidebarContent>

      {/* -------- Footer -------- */}
      {(username && token) && (
        <SidebarFooter>
          <NavUser user={user_info} />
        </SidebarFooter>
      )}

      <SidebarRail />
    </Sidebar>
  );
}
