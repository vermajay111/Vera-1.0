...
from promises.models import Promise, PromiseReceiver
...
def user_profile_info(request):
    ...
    avatar_url = user.avatar_url if user.avatar_url else user.deafult_avatar_url
...
def user_dashboard(request):
    ...
    for promise in active_promises_qs:
        particpiants = []
        recivers = PromiseReceiver.objects.filter(promise=promise)
        for reciver in recivers:
            particpiants.append({
                "username": reciver.reciver.username,
                "avatar_url": reciver.reciver.avatar_url or reciver.deafult_avatar_url,
                "id": reciver.reciver.id,
                "accepted": reciver.accepted,
                "completed": reciver.complete,
                "responded_at": reciver.responded_at
            })
        ...
        active_promises.append({
            ...
            "majority_vote": promise.majority_vote,
            ...
            "particpiants": particpiants,
            "owner": promise.giver,
            "owner_id": promise.giver.id
        })
